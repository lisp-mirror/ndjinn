...
(defun pbr-mesh/vertex ((mesh-attrs mesh-attrs)
                        &uniforms
                        (model :mat4)
                        (view :mat4)
                        (proj :mat4)
...
(defun pbr-mesh/fragment ((world-pos :vec3)
                          (view-dir :vec3)
                          (normal :vec3)
                          (uv :vec2)
                          &uniforms
...
(define-shader pbr-mesh ()
  (:vertex (pbr-mesh/vertex mesh-attrs))
  (:fragment (pbr-mesh/fragment :vec3 :vec3 :vec3 :vec2)))
