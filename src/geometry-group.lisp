  ...
  (let ((groups (u:dict))
    ...
    (dolist (group spec)
      (destructuring-bind (name (&key (format 'interleaved) (divisor 0))
                           . attrs)
          group
        (u:mvlet ((group-type (a:format-symbol :pyx "GEOMETRY-GROUP/~a" format))
  ...
  (with-slots (%attributes %attribute-order %divisor) group
    (loop :for attr-name :in %attribute-order
          ...
          :do (gl:bind-buffer :array-buffer buffer)
              (configure-geometry-attribute attr i 0 0 %divisor))))

(defmethod configure-geometry-group ((group geometry-group/interleaved)
                                     index buffers)
  (with-slots (%attributes %attribute-order %divisor) group
    ...
    (loop :with stride = (get-geometry-group-attribute-size group)
          ...
          :for attr = (u:href %attributes attr-name)
          :for i :from index
          :do (configure-geometry-attribute attr i stride offset %divisor)
