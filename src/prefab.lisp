...
(defun register-prefab-viewports (entity &key viewports)
  (let ((viewports-table (viewport-manager-table
...
(defun load-prefab (name &rest args &key viewports parent &allow-other-keys)
  (let ((prefabs =meta/prefabs=))
    (u:if-let ((prefab (u:href prefabs name)))
      (let* ((args (u:plist-remove args :viewports :parent))
             (factory (factory (u:href prefabs name)))
             (entity (funcall (func factory) args :parent parent)))
...
(defun deregister-prefab-entity (entity)
  (u:when-let* ((prefab (node/prefab entity))
                (table (scene-prefabs (current-scene =context=))))
...
(on-recompile :prefab data ()
  (dolist (entity (u:href (scene-prefabs (current-scene =context=)) data))
